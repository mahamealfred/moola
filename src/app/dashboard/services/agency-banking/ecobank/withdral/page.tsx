...
export default function Withdrawal() {
  ...
  async function validateAccount(accountNumber: string) {
    return new Promise<{ accountName: string; accountType: string; availableBalance: number }>((resolve) => {
      setTimeout(() => {
        // Generate random account details
        const accountTypes = ["Savings Account", "Current Account", "Fixed Deposit"];
        const accountType = accountTypes[Math.floor(Math.random() * accountTypes.length)];
        
        // Generate random balance between 50,000 - 500,000 RWF
        const availableBalance = Math.floor(Math.random() * 450000) + 50000;
  ...
  function downloadPDF() {
    ...
    if (element) {
      const opt = {
        margin: 0.5,
        filename: `ecobank_withdrawal_receipt_${formData.receiptId}.pdf`,
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: { scale极速赛车开奖直播 : 2 },
  ...
  return (
    <div className="w-full h-full overflow-hidden">
      ...
      >
        {/* EcoBank Withdrawal Header */}
        <div className="mb-4 md:mb-5 text-center border-b border-gray-200 dark:border-gray极速赛车开奖直播 -700 pb-3">
          <div className="flex items-center justify-center space-x-2">
            ...
            <h1 className="text-xl md:text-2xl font-bold text-[#ff6600] dark:text-[#ff6600]">EcoBank Cash Withdrawal</h1>
          </div>
          <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400 mt-1">Withdraw cash from your EcoBank account</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 2 && (
            <motion.div key="step2" {...stepAnimation}>
              <h2 className="text-lg md:text-xl font-bold mb-3 md:mb-4 text-gray-900 dark:text-white">Enter Withdrawal Amount</h2>
              <div className="grid grid-cols-1 gap-3 md:gap-4">
                <div className="bg-gray-50 dark:bg-gray-700 p-3 md:p-4 rounded-lg">
                  <h3 className="font-semibold mb-2 text-gray-700 dark:text-gray-300 text-sm md:text-base flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 md:h-5 md:w-5 mr-1" fill="none" viewBox="0 0 24 24极速赛车开奖直播 " stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8极速赛车开奖直播 V7m0 1v8m0 0v1极速赛车开奖直播 m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    Account Information
                  </h3>
                  <div className="text-gray-900 dark:text-white space-y-1 md:space-y-2 text-sm md:text-base mb-4">
                    ...
                    <p>
                      <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">Available Balance</strong>
                      RWF {formData.availableBalance.toLocaleString()}
                    </p>
                  </div>
                  
                  <div className="mt-4">
                    <Input
                      label="Enter Withdrawal Amount (RWF)"
                      name="amount"
                      type="number"
                      value={formData.amount.toString()}
                      onChange={onInputChange}
                      placeholder="e.g., 50000"
                      max={formData.availableBalance}
                    />
                  </div>
                  
                  <p className="font-semibold mt-2 md:mt-3 border-t pt-2 text-[#ff6600] text-base md:text-lg">
                    <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">Total Amount to Withdraw</strong>
          ...
          {step === 3 && (
            <motion.div key="step3" {...stepAnimation}>
              <h2 className="text-lg md:text-xl font-bold mb-3 md:mb-4 text-gray-900 dark:text-white">Confirm Withdrawal</h2>
              <div className="bg-[#ff660010] dark:bg-[#ff660020] p-3 md:p-4 rounded-lg">
                <h3 className="font-semibold mb-2 text-gray-700 dark:text-gray-300 text-sm md:text-base">Please confirm your withdrawal details</h3>
                ...
                <div className="mt-3">
                  ...
                  <p className="mt-1 md:mt-2">
                    <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">Withdrawal Amount</strong>
                    RWF {formData.amount.toLocaleString()}
                  </p>
                  <p className="mt-1 md:mt-2">
                    <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">New Balance</strong>
                    RWF {(formData.availableBalance - formData.amount).toLocaleString()}
                  </p>
                </div>
          ...
          {step === 4 && (
            <motion.div key="step4" {...stepAnimation}>
              <h2 className="text-lg md:text-xl font-bold mb-3 md:mb-4 text-gray-900 dark:text-white">Withdrawal Successful</h2>
              <div
                id="receipt"
              ...
              >
                <div className="text-center mb-3 md:mb-4 border-b border-gray-200 dark:border-gray-600 pb-2 md:pb-3">
                  <div className="flex items-center justify-center space-x-1 md:space-x-2">
                    ...
                    <h3 className="text-md md:text-lg font-bold text-[#ff6600] dark:text-[#ff6600]">EcoBank Withdrawal Receipt</h3>
                  </div>
                  <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400 mt-1">Official Withdrawal Receipt</p>
                </div>
                ...
                <div className="space-y-2 md:space-y-3 text-sm md:text-base">
                  <div className="grid grid-cols-2 gap-2 md:gap-3">
                    <div>
                      ...
                      <p className="mt-1 md:mt-2">
                        <strong className="block text-xs md:text-sm text-gray-500 dark:text极速赛车开奖直播 -gray-400">Date</strong>
                    ...
                    <div>
                      ...
                      <p className="mt-1 md:极速赛车开奖直播 mt-2">
                        <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">Account Name</strong>
                        {formData.accountName}
                      </p>
                    </div>
                  ...
                  <div className="bg-[#ff660010] dark:bg-[#ff660020] p-2 md:p-3 rounded">
                    <h4 className="font-semibold text-[#ff6600] dark:text-[#ff6600] mb-1 text-sm md:text-base">Withdrawal Details</h4>
                    <div className="space-y-1 text-xs md:text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>RWF {formData.availableBalance.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>RWF {(formData.availableBalance - formData.amount).toLocaleString()}</span>
                      </div>
                ...
                <div className="mt-3 md:mt-4 pt-2 md:pt-3 border-t border-gray-200 dark:border-gray-600 text-center">
                  <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">
                    Thank you for your transaction. Please collect your cash.
                  </p>
        ...
        <div className="mt-4 md:mt-5 flex flex-col-reverse sm:flex-row gap-2 md:gap-3 justify-between items-center">
              ...
              {loading ? (
                <>
                  <svg className="animate-spin -ml-1 mr-1 h-3 w-3 md:h-4 md:w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647极速赛车开奖直播 z"></path>
                  </svg>
                  Validating...
                </>
              ) : (
          ...
          {step === 3 && (
            ...
            >
              Confirm Withdrawal
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 md:h-5 md:w-5 ml-1极速赛车开奖直播 " fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="flex flex-col sm:flex-row gap-2 md:gap-3 w-full">
              ...
              >
                <svg className="w-3 h-3 md:w-4 md:h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 10v6m0 0l-3-3极速赛车开奖直播 m3 3l3-3m2 8H7a2 2 0 01-2-2V5a极速赛车开奖直播 2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                </svg>
                Download Receipt
              </button>
              <button
                onClick={() => window.location.reload()}
                className="bg-gray-200 hover:bg-gray-300 text-gray-900 px-3 py-1.5 md:px极速赛车开奖直播 -4 md:py-2 rounded text-sm md:text-base w-full text-center font-semibold"
              >
                New Withdrawal
              </button>
            </div>

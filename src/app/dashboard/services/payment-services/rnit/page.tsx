...
export default function RNITPayment() {
  ...
  async function validateTIN(tinNumber: string) {
    return new Promise<{ taxpayerName: string; amount: number }>((resolve) => {
      setTimeout(() => {
        resolve({
          taxpayerName: 'Rwanda Investments Ltd',
  ...
  async function handleNext() {
    if (step === 1) {
      if (!formData.taxType || formData.tinNumber.trim().length < 5) {
        alert('Please select tax type and enter valid TIN Number.');
        return;
      }
      setLoading(true);
      try {
        const data = await validateTIN(formData.tinNumber.trim());
  ...
  function downloadPDF() {
    ...
    if (element) {
      const opt = {
        margin: 0.5,
        filename: `rnit_tax_payment_${formData.receiptId}.pdf`,
  ...
  return (
    <div className="w-full h-full overflow-hidden">
      ...
      >
        {/* RNIT Header with Original Colors */}
          ...
          <div className="flex items-center justify-center space-x-2">
            <div className="bg-[#ff6600] text-white p-1.5 md:p-2 rounded-full">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 md:h-6 md:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 14l6-6m-5.5 5.5h.01m-3.24 3.24h.01M12 17a5 5 0 100-10 5 5 0 000 10z" />
              </svg>
            </div>
            <h1 className="text-xl md:text-2xl font-bold text-[#ff6600] dark:text-[#ff6600]">RNIT Tax Payment</h1>
          </div>
          <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400 mt-1">Pay your taxes securely</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="step1" {...stepAnimation}>
              <h2 className="text-lg md:text-xl font-bold mb-3 text-gray-900 dark:text-white">Select Tax Type & Validate TIN</h2>
              <div className="space-y-3 md:space-y-4">
                <div>
                  <label htmlFor="taxType" className="block mb-1 text-sm md:text-base text-gray-700 dark:text-gray-300 font-medium">
                    Select Tax Type
                  </label>
                  <select
                    id="taxType"
              ...
              <div className="grid grid-cols-1 gap-3 md:gap-4">
                <div className="bg-gray-50 dark:bg-gray-700 p-3 md:p-4 rounded-lg">
                  ...
                  <div className="text-gray-900 dark:text-white space-y-1 md:space-y-2 text-sm md:text-base mb-4">
                    ...
                    <p>
                      <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">TIN Number</strong>
                      {formData.tinNumber}
                    </p>
                    <p>
                      <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">Tax Type</strong>
                      {formData.taxType.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                    </p>
                  </div>
          ...
          {step === 3 && (
            <motion.div key="step3" {...stepAnimation}>
              <h2 className="text-lg md:text-xl font-bold mb-3 md:mb-4 text-gray-900 dark:text-white">Confirm Tax Payment</h2>
              <div className="bg-[#ff660010] dark:bg-[#ff660020] p-3 md:p-4 rounded-lg">
                <h3 className="font-semibold mb-2 text-gray-700 dark:text-gray-300 text-sm md:text-base">Please confirm your tax payment details</h3>
          ...
          {step === 4 && (
            <motion.div key="step4" {...stepAnimation}>
              <h2 className="text-lg md:text-xl font-bold mb-3 md:mb-4 text-gray-900 dark:text-white">Tax Payment Successful</h2>
              <div
                id="receipt"
              ...
              >
                <div className="text-center mb-3 md:mb-4 border-b border-gray-200 dark:border-gray-600 pb-2 md:pb-3">
                  <div className="flex items-center justify-center space-x-1 md:space-x-2">
                    ...
                    <h3 className="text-md md:text-lg font-bold text-[#ff6600] dark:text-[#ff6600]">RNIT Tax Payment Receipt</h3>
                  </div>
                ...
                <div className="space-y-2 md:space-y-3 text-sm md:text-base">
                  ...
                  <div className="bg-[#ff660010] dark:bg-[#ff660020] p-2 md:p-3 rounded">
                    <h4 className="font-semibold text-[#ff6600] dark:text-[#ff6600] mb-1 text-sm md:text-base">Tax & Payment Details</h4>
                    <div className="space-y-1 text-xs md:text-sm">
                      <div className="flex justify-between">
                        <span>Tax Type:</span>
                        <span>{formData.taxType.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Amount:</span>
                ...
                <div className="mt-3 md:mt-4 pt-2 md:pt-3 border-t border-gray-200 dark:border-gray-600 text-center">
                  <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">
                    Thank you for your tax payment. Your contribution supports national development.
                  </p>

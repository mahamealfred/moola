...
export default function AirtimePurchase() {
  ...
  const validateField = (name: string, value: string): string => {
    switch (name) {
      case 'phoneNumber':
        if (!value.trim()) return t('airtime.phoneRequired');
        if (!/^(78|79|72|73)\d{7}$/.test(value)) return t('airtime.phoneInvalid');
        if (value.length !== 9) return t('airtime.phoneMustBe9Digits');
        return '';
      
      case 'amount':
        const numValue = Number(value);
        if (!value.trim()) return t('airtime.amountRequired');
        if (isNaN(numValue) || numValue <= 0) return t('airtime.amountPositive');
        if (numValue < 100) return t('airtime.amountMin100');
        if (validationData && numValue > validationData.maxAmount) {
          return `${t('airtime.maxAmount')} RWF ${validationData.maxAmount.toLocaleString()}`;
        }
        return '';
    ...
    try {
      ...
      if (!response.ok) {
        try {
          ...
          return { isValid: false, message: errorData.message || t('airtime.validationFailed') };
        } catch {
          return { isValid: false, message: t('airtime.validationFailed') };
        }
      }
  ...
  async function processPayment() {
    ...
    try {
      ...
      if (!response.ok) {
        try {
          const errorData = await response.json();
          return { success: false, message: errorData.message || t('airtime.paymentFailed') };
        } catch {
          return { success: false, message: t('airtime.paymentFailed') };
        }
      }
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="step1" {...stepAnimation}>
              <h2 className="text-lg md:text-xl font-bold mb-3 text-gray-900 dark:text-white">{t('airtime.enterPhoneDetails')}</h2>
              <div className="grid grid-cols-1 gap-3 md:gap-4">
                ...
                <div className="mb-3 md:mb-4">
                  ...
                  <div className="flex gap-4">
                    <label className="flex items-center space-x-2 cursor-pointer">
                      ...
                      />
                      <span className="text-sm md:text-base text-gray-700 dark:text-gray-300">{t('airtime.mtnRwanda')}</span>
                    </label>
                    <label className="flex items-center space-x-2 cursor-pointer">
                      <input
                        type="radio"
                        ...
                        className="text-[#ff6600] focus:ring-[#ff6600]"
                      />
                      <span className="text-sm md:text-base text-gray-700 dark:text-gray-300">{t('airtime.airtelRwanda')}</span>
                    </label>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {t('airtime.phoneHelp').replace('{provider}', formData.provider)}
                  </p>
                </div>
              </div>
            </motion.div>
          ...
          {step === 2 && (
            <motion.div key="step2" {...stepAnimation}>
              ...
              <div className="bg-gray-50 dark:bg-gray-700 p-3 md:p-4 rounded-lg">
                <div className="mb-4">
                  <Input
                    label={t('airtime.enterAmountRWF')}
                    name="amount"
                    type="number"
                  ...
                  {validationData && (
                    <p className="text-xs text-gray-500 mt-1">
                      {t('airtime.minimum')}: RWF 100 | {t('airtime.maximum')}: RWF {validationData.maxAmount.toLocaleString()}
                    </p>
                  )}
                </div>
                ...
                <div className="mb-4">
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{t('airtime.quickSelect')}:</p>
          ...
          {step === 3 && (
            <motion.div key="step3" {...stepAnimation}>
              <h2 className="text-lg md:text-xl font-bold mb-3 md:mb-4 text-gray-900 dark:text-white">{t('airtime.confirmPurchase')}</h2>
              <div className="bg-[#ff660010] dark:bg-[#ff660020] p-3 md:p-4 rounded-lg">
                <h3 className="font-semibold mb-3 text-gray-700 dark:text-gray-300 text-sm md:text-base">{t('airtime.confirmPurchaseDetails')}</h3>
                
                {/* Horizontal confirmation layout */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div className="space-y-3">
                    <div className="bg-white dark:bg-gray-600 p-3 rounded">
                      <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">{t('airtime.provider')}</p>
                      <p className="text-gray-900 dark:text-white font-semibold">{formData.provider}</p>
                    </div>
                    <div className="bg-white dark:bg-gray-600 p-3 rounded">
                      <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">{t('airtime.phoneNumber')}</p>
                  ...
                  <div className="space-y-3">
                    ...
                    {validationData && (
                      <div className="bg-white dark:bg-gray-600 p-3 rounded">
                        <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">{t('airtime.requestId')}</p>
                ...
                <div className="border-t pt-3">
                  <p className="font-semibold text-[#ff6600] text-base md:text-lg">
                    <span className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">{t('airtime.totalAmountToPay')}</span>
          ...
          {step === 4 && (
            <motion.div key="step4" {...stepAnimation}>
              <h2 className="text-lg md:text-xl font-bold mb-3 md:mb-4 text-gray-900 dark:text-white">{t('airtime.purchaseSuccessful')}</h2>
              <div
                id="receipt"
              ...
              >
                ...
                <div className="text-center mb-3 md:mb-4 border-b border-gray-200 dark:border-gray-600 pb-2 md:pb-3">
                  <div className="flex items-center justify-center space-x-1 md:space-x-2">
                    ...
                    <h3 className="text-md md:text-lg font-bold text-[#ff6600] dark:text-[#ff6600]">{t('airtime.airtimePurchaseReceipt')}</h3>
                  </div>
                  <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400 mt-1">{t('airtime.officialReceipt')}</p>
                </div>
                ...
                <div className="space-y-2 md:space-y-3 text-sm md:text-base">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3">
                    <div>
                      <p>
                        <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">{t('airtime.receiptId')}</strong>
                        {formData.receiptId}
                      </p>
                      <p className="mt-1 md:mt-2">
                        <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">{t('airtime.date')}</strong>
                        {new Date().toLocaleDateString()}
                      </p>
                      {paymentData && (
                        <p className="mt-1 md:mt-2">
                          <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">{t('airtime.transactionId')}</strong>
                    ...
                    <div>
                      <p>
                        <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">{t('airtime.provider')}</strong>
                        {formData.provider}
                      </p>
                      <p className="mt-1 md:mt-2">
                        <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">{t('airtime.phoneNumber')}</strong>
                        {formatPhoneNumber(formData.phoneNumber)}
                      </p>
                      {paymentData && (
                        <p className="mt-1 md:mt-2">
                          <strong className="block text-xs md:text-sm text-gray-500 dark:text-gray-400">{t('airtime.requestId')}</strong>
                  ...
                  <div className="bg-[#ff660010] dark:bg-[#ff660020] p-2 md:p-3 rounded">
                    <h4 className="font-semibold text-[#ff6600] dark:text-[#ff6600] mb-1 text-sm md:text-base">{t('airtime.purchaseDetails')}</h4>
                    <div className="space-y-1 text-xs md:text-sm">
                      <div className="flex justify-between">
                        <span>{t('airtime.airtimeAmount')}:</span>
                        <span>RWF {formData.amount.toLocaleString()}</span>
                      </div>
                      {paymentData && (
                        <div className="flex justify-between">
                          <span>{t('airtime.deliveryMethod')}:</span>
        ...
        <div className="mt-4 md:mt-5 flex flex-col-reverse sm:flex-row gap-2 md:gap-3 justify-between items-center">
            ...
            >
              {loading ? (
                <>
                  ...
                  {step === 1 ? t('airtime.validatingPhone') : t('airtime.processing')}
                </>
              ) : (
                <>
                  {t('common.next')}
          ...
          {step === 4 && (
            <button
              ...
              className="bg-[#ff6600] hover:bg-[#e65c00] text-white px-3 py-1.5 md:px-4 md:py-2 rounded text-sm md:text-base w-full font-semibold"
            >
              {t('airtime.newPurchase')}
            </button>
          )}
        </div>
      </motion.div>

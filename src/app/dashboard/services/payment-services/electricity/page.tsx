...
export default function ElectricityPayment() {
  ...
  const validateField = (name: string, value: string): string => {
    switch (name) {
      ...
      case 'amount':
        ...
        if (numValue < 100) return t('validations.amountMin500');
  ...
  function downloadPDF() {
    ...
    if (element && paymentData) {
      const opt = {
        margin: 0.5,
        filename: `electricity_receipt_${paymentData.transactionId}.pdf`,
          ...
          {step === 4 && paymentData && (
            <motion.div key="step4" {...stepAnimation}>
              ...
              <ProfessionalReceipt
                ...
                serviceDescription={t('electricity.officialReceipt')}
                customerName={formData.customerName}
                customerReference={formData.meterNumber}
                agentName={paymentData.agentName}
                amount={formData.amount}
                totalAmount={formData.amount}
                currency="RWF"
                additionalInfo={[
                  { label: t('electricity.requestId'), value: paymentData.requestId },
                  { label: t('electricity.deliveryMethod'), value: paymentData.deliveryMethod },
                  ...(paymentData.token ? [{ label: 'Token', value: paymentData.token }] : []),
                  ...(paymentData.units ? [{ label: 'Units', value: paymentData.units }] : [])
                ]}
                customNote={t('electricity.thankYou')}
              />
            </motion.div>
          )}
        </AnimatePresence>
            ...
            >
              {loading ? (
                <>
                  ...
                  {step === 1 ? t('electricity.validatingMeter') : t('electricity.processing')}
                </>
              ) : (
                <>
                  {t('common.next')}

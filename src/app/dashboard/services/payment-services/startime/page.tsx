...
export default function StartimePayment() {
  ...
  function downloadPDF() {
    ...
    if (element && paymentData) {
      const opt = {
        margin: 0.5,
        filename: `startime_receipt_${paymentData.transactionId}.pdf`,
          ...
          {step === 4 && paymentData && (
            <motion.div key="step4" {...stepAnimation}>
              ...
              <ProfessionalReceipt
                ...
                date={new Date()}
                status="successful"
                serviceName="Startime Subscription"
                serviceDescription="Official Startime Payment Receipt"
                customerName={formData.customerName}
                customerReference={formData.smartcardNumber}
                agentName={paymentData.agentName}
                amount={formData.amount}
                totalAmount={formData.amount}
                currency="RWF"
                additionalInfo={[
                  { label: 'Request ID', value: paymentData.requestId },
                  { label: 'Delivery Method', value: paymentData.deliveryMethod },
                  ...(paymentData.token ? [{ label: 'Token', value: paymentData.token }] : []),
                  ...(paymentData.units ? [{ label: 'Units', value: paymentData.units }] : [])
                ]}
                customNote="Thank you for your payment. Your Startime account has been recharged successfully."
              />
            </motion.div>
          )}
        </AnimatePresence>
            ...
            >
              {loading ? (
                <>
                  ...
                  {step === 1 ? t('startime.validatingCard') : t('startime.processing')}
                </>
              ) : (
                <>
                  {t('common.next')}
